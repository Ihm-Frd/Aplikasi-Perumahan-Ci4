...
function encme($str=null,$key='SecretCode'){
	$huruf="qazWQAZ012xswnjmMNedcvDCXS3789kio@lpPOfrtgbHY-TGB456VFREL_hyuKIUJ";
	($key=='SecretCode')?$key=SKID:null;
	if($str!=null){
		$hsl='';$str=base64_encode($str);
		$pstr=strlen($str);
		$ph=strlen($huruf);
		$keyl=cstr($str,$key);
		for ($i=0; $i < $pstr; $i++) { 
			$keS=strpos($huruf,$str[$i]);
			$keK=strpos($huruf,$keyl[$i]);
			if($keS!==false){
				$n=$keS+$keK;
				$hsl.=$huruf[$n%$ph];
			}else{$hsl.=$str[$i];}
...
function desme($str=null,$key='SecretCode'){
	$huruf="qazWQAZ012xswnjmMNedcvDCXS3789kio@lpPOfrtgbHY-TGB456VFREL_hyuKIUJ";
	($key=='SecretCode')?$key=SKID:null;
	if($str!=null){
		$hsl='';
		...
		for ($i=0; $i < $pstr; $i++) { 
			$keS=strpos($huruf,$str[$i]);
			$keK=strpos($huruf,$keyl[$i]);
			// $hsl.='<hr>'.$i.'. Str='.$str[$i].'<br>Key='.$keyl[$i].'<br>KeS='.$keS.'<br>KeK='.$keK;
			if($keS!==false){
				$n=$keS-$keK;
				// $hsl.='<br>n-1='.$n;
				if($n < 0){
				...
				$hsl.=$huruf[$n%$ph];
				// $hsl.='<br>hasil='.$huruf[$n%$ph];
			}else{$hsl.=$str[$i];}
		}
		return base64_decode($hsl);
		// return $hsl;
	}else{return null;}
}

function cstr($str,$key){
	$pstr=strlen($str);
	$pkey=strlen($key);
	if($pkey < $pstr){$key.=$key;}
	
	if(strlen($key) < $pstr){return cstr($str,$key);}
	else{return $key;}
}